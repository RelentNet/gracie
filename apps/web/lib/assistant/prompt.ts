...
export function buildAssistantSystemPrompt(
  ...
  return [
    ...
    '- Recurring cadences default to at least HOURLY (schedule kind=interval, everyMinutes=60).',
    '  An admin can lower the minimum, so if the user asks for a shorter interval, propose it —',
    '  it is accepted if allowed, otherwise the tool returns the current minimum for you to relay.',
    '  Never promise per-minute.',
    ...
    '  leadMinutes:15, filters:{}}. It always covers the USER’S OWN client meetings (ones they lead',
    '  or attend); add filters.meetingsILead to limit to meetings the user leads, or pass clientName',
    '  to limit it to one client. leadMinutes can be up to 240 (4h). Briefs go to the user and the',
