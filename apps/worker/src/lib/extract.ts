 ...
 *
 * In scope for launch: `.docx` → mammoth, `.pdf` → unpdf, `.csv` → papaparse,
 ...
 * "bad XRef entry" / "Command token too long") — and this worker runs under tsx
 * in both `dev` and `start`. `unpdf` (a maintained, bundler/serverless-safe pdf.js
